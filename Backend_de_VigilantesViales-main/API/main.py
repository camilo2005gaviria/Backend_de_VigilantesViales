...
@app.get(
    ...
    response_model=List[schemas.PorDiaSemanaOut],
    tags=["Estadísticas"],
    summary="Accidentes agrupados por día de la semana",
)
def accidentes_por_dia_semana(db: Session = Depends(get_db)):
    """Accidentes y víctimas fatales por día de la semana."""
    rows = (
        db.query(
            models.Accidente.fecha,
            func.count(models.Accidente.id_accidente).label("total"),
        )
        .group_by(models.Accidente.fecha)
        .all()
    )

    # Agrupar por día de la semana en Python
    ...
    for fecha, total in rows:
        ...
        dia_nombre = dias[fecha.strftime("%A")]
        if dia_nombre not in totals:
            totals[dia_nombre] = {"total": 0, "fatales": 0}
        totals[dia_nombre]["total"] += total

    # Contar fatales
    graves = (
        db.query(
            models.Accidente.fecha,
            func.count(models.Accidente.id_accidente).label("count"),
        )
        .join(models.GravedadAccidente)
        .filter(models.GravedadAccidente.nivel.ilike("%alto%"))
        .group_by(models.Accidente.fecha)
        .all()
    )

    for fecha, count in graves:
        if isinstance(fecha, str):
            fecha = datetime.strptime(fecha, "%Y-%m-%d")
        dia_nombre = dias[fecha.strftime("%A")]
        if dia_nombre in totals:
            totals[dia_nombre]["fatales"] += count

    return [
        schemas.PorDiaSemanaOut(dia=dia, total=totals[dia]["total"], fatales=totals[dia]["fatales"])
        for dia in orden if dia in totals
    ]


@app.get(
    "/estadisticas/tendencia-anual",
    response_model=List[schemas.TendenciaAnualOut],
    tags=["Estadísticas"],
    summary="Tendencia anual de accidentes",
)
def tendencia_anual(db: Session = Depends(get_db)):
    """Accidentes y víctimas fatales por año."""
    rows = (
        db.query(
            extract("year", models.Accidente.fecha).label("anio"),
            func.count(models.Accidente.id_accidente).label("total"),
        )
        .group_by("anio")
        .order_by("anio")
    ...
    fatales_por_anio = {row[0]: row[1] for row in graves}

    return [
        schemas.TendenciaAnualOut(anio=int(r[0]), accidentes=r[1], fatales=fatales_por_anio.get(int(r[0]), 0))
        for r in rows
    ]
...
@app.get(
    ...
    response_model=List[schemas.PorMarcaOut],
    tags=["Estadísticas"],
    summary="Accidentes por marca de vehículo",
)
def accidentes_por_marca(db: Session = Depends(get_db)):
    """Accidentes y fatales agrupados por marca de vehículo."""
    rows = (
        db.query(
            models.Marca.nombre_marca,
            func.count(models.Accidente.id_accidente).label("total"),
        )
        .join(models.Vehiculo, models.Vehiculo.id_vehiculo == models.Accidente.id_vehiculo)
        .join(models.Marca, models.Marca.id_marca == models.Vehiculo.id_marca)
        .group_by(models.Marca.nombre_marca)
        .order_by(func.count(models.Accidente.id_accidente).desc())
        .all()
    )

    # Contar fatales por marca
    graves = (
        db.query(
            models.Marca.nombre_marca,
            func.count(models.Accidente.id_accidente).label("count"),
    ...
    fatales_por_marca = {r[0]: r[1] for r in graves}

    return [
        schemas.PorMarcaOut(nombre_marca=r[0], cantidad=r[1], fatal=fatales_por_marca.get(r[0], 0))
        for r in rows
    ]


@app.get(
    "/estadisticas/por-autoridad",
    response_model=List[schemas.PorAutoridadOut],
    tags=["Estadísticas"],
    summary="Accidentes por autoridad de tránsito",
)
def accidentes_por_autoridad(db: Session = Depends(get_db)):
    """Accidentes agrupados por autoridad de tránsito."""
    rows = (
        db.query(
            models.AutoridadTransito.nombre_autoridad,
            func.count(models.Accidente.id_accidente).label("total"),
        )
        .join(models.AutoridadTransito)
        .group_by(models.AutoridadTransito.nombre_autoridad)
        .order_by(func.count(models.Accidente.id_accidente).desc())
        .all()
    )

    return [
        schemas.PorAutoridadOut(nombre_autoridad=r[0], accidentes=r[1])
        for r in rows
    ]
